...
export class ChatAiService {
  ...
  async generateAdminReply(prompt: string, analyticsContext: string): Promise<string> {
    ...
    const systemPrompt = [
      ...
      "If requested data is not in context, clearly say it is unavailable and never guess.",
      ...
      "Use STORE_SNAPSHOT_JSON for detailed answers about orders, users, products, conversations, and messages.",
      "When asked for order descriptions/details, include order id, status, buyer, total, and item breakdown from STORE_SNAPSHOT_JSON.",
      "Prefer complete answers from available context instead of saying data is unavailable.",
